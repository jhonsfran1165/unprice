    ...
    <div className={cn("w-full space-y-2.5 overflow-auto", className)} {...skeletonProps}>
        ...
        <div className="flex flex-1 items-center space-x-2">
          {searchableColumnCount > 0
            ? Array.from({ length: searchableColumnCount }).map((_, i) => (
                <Skeleton key={i.toString()} className="h-7 w-40 lg:w-60" />
              ))
            : null}
          {filterableColumnCount > 0
            ? Array.from({ length: filterableColumnCount }).map((_, i) => (
                <Skeleton key={i.toString()} className="h-7 w-[4.5rem] border-dashed" />
              ))
            : null}
        </div>
      ...
      {withPagination ? (
        <div className="flex w-full items-center justify-between gap-4 overflow-auto p-1 sm:gap-8">
          ...
          <div className="flex items-center gap-4 lg:gap-8 sm:gap-6">

  ...
  return (
    ...
    <li
      className={cn(
        ...
        hasOnValueChange ? "cursor-pointer hover:bg-background-bgHover" : "cursor-default"
      )}
      onClick={(e) => {
        e.stopPropagation()
        onClick?.(name, color)
    ...
    >
      ...
      <p
        className={cn(
          ...
          hasOnValueChange && "group-hover:text-background-textContrast",
          activeLegend && activeLegend !== name ? "opacity-40" : "opacity-100"
        )}
      >
        {name}
...
const ChartTooltipRow = ({ value, name, color }: ChartTooltipRowProps) => (
  <div className="flex items-center justify-between space-x-8">
    <div className="flex items-center space-x-2">
      <span aria-hidden="true" className={cn("h-[3px] w-3.5 shrink-0 rounded-full", color)} />
      <p
        className={cn(
          // commmon
    ...
    <p
      className={cn(
        ...
        // text color
        "bg-background-bg text-background-text"
      )}
    >
      {value}
    </p>
...
type TooltipCallbackProps = Pick<ChartTooltipProps, "active" | "payload" | "label">
    ...
    return (
      ...
      >
        ...
        <div className={cn("space-y-1 px-4 py-2")}>
          {filteredPayload.map(
            ({ value, name }: { value: number; name: string }, index: number) => (
              <ChartTooltipRow
                // biome-ignore lint/suspicious/noArrayIndexKey: <explanation>
                key={`id-${index}`}
    ...
    <div ref={ref} className={cn("h-80 w-full", className)} {...other}>
      <ResponsiveContainer width={"100%"} height={"100%"}>
        ...
        >
          ...
          >
            {xAxisLabel && (
              <Label position="insideBottom" offset={-20} className="fill-background-text">
                {xAxisLabel}
              </Label>
            )}
          </XAxis>

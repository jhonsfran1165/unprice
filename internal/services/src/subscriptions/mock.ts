import { Database } from "@unprice/db"
import { subscriptionPhases, subscriptions } from "@unprice/db/schema"
import { configureBillingCycleSubscription, Customer, Subscription, SubscriptionPhaseExtended } from "@unprice/db/validators"
    ...
    },
    {
      ...
      featurePlanVersion: {
        ...
        config: {
          ...
          tiers: [
            {
              unitPrice: { dinero: { amount: 0, currency: { code: "USD", base: 10, exponent: 2 }, scale: 2 }, displayAmount: "0.00" },
              flatPrice: { dinero: { amount: 0, currency: { code: "USD", base: 10, exponent: 2 }, scale: 2 }, displayAmount: "0.00" },
              firstUnit: 1,
              lastUnit: 100
            },
            {
              unitPrice: { dinero: { amount: 1, currency: { code: "USD", base: 10, exponent: 2 }, scale: 2 }, displayAmount: "0.01" },
              flatPrice: { dinero: { amount: 0, currency: { code: "USD", base: 10, exponent: 2 }, scale: 2 }, displayAmount: "0.00" },
              firstUnit: 101,
              lastUnit: 1000
            }
...
export const createMockSubscription = ({
  ...
  const nextInvoiceAt = mockPhase.whenToBill === "pay_in_advance" ? calculatedBillingCycle.cycleStart.getTime() : calculatedBillingCycle.cycleEnd.getTime()
  ...
  const mockSubscription: Subscription = {
    ...
    nextInvoiceAt: mockPhase.trialDays > 0 ? calculatedBillingCycle.cycleEnd.getTime() : nextInvoiceAt
  } as Subscription
